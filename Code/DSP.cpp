...
#define NUM_AP 100
#define NUM_ES 30
#define NUM_CHUNK 5000 //movies
#define ES_CAP 50 //blockcount
#define MAX_DIST 15
#define NUM_Req 100000
#define MAXSIMTIME 200000
...
#define Replace 0 //0-LRU,1-LFU,2-Zipfs

#define LOCAL_ACCESS_TIME 3
#define SHARER_ACCESS_TIME 5
#define Miss2Cloud  30

#define LOCAL_SHARE 1 
//1-sharing, 0-no share

#define FWD 0.1 //0-full private, 1-full public, so this is decimal bt 0 to 1, our case .7, 0-no split, 

#define DEADLINE 0
#define PROFIT 0
#define SORT 0 //1-fcfs
...
class ES_Class{
     ...
     int ES_ID;
     void Clear(){
	     for(int i=0;i<ES_CAP;i++) {DC[i]=-1;LRU[i]=-1;Freq[i]=-1; hit=0;miss=0;FreqG[i]=-1;}
     }
     void Schedule(){}
...
void ES_Class::Place(int _cid, int local){
	     ...
	     {
	     	int flag=0;
	     	int iLRU = start, vLEU = LRU[start];
	     	if(DEBUG) cout<<"\nplace local="<<local<<" start= "<<start<<" end= " <<end<<endl;

	     	for(int i=start;i<end;i++){
		    	if (LRU[i]==-1) { iLRU=i; flag = 1; break; }  	
	       	}
	       	if(flag == 0)
	       	{
	       		float RV[ES_CAP];
	       		...
	       		{
	       			float c = 5/30;
	       			int tstay = sim_time - LRU[iLRU];
	       			int tcref = sim_time;
	       			int tref = LRU[iLRU];
	       			float age = tstay/(tcref-tref);
	       			RV[i] = age + c * pow(double((Freq[i] / FreqG[i])),double(1/.7)) ; 
	       		}
...
int ES_Class::Access(Req *R, int LocalAccess){
	 ...
	 if(FWD && LocalAccess) {
		...
		if (X&&Y) {
			   if((random()%100)>50) ES[left].Access(R,1); else ES[right].Access(R,1); return 0;
			  }
		if (X) { ES[left].Access(R,1);return 0;}	
		if (Y) { ES[right].Access(R,1);return 0;}

	        if ((random()%100)>(LOCAL_SHARE*100)){ //Forward miss to Sharer with probablity 1-LOCAL_SHARE
			if((random()%100)>50) ES[left].Access(R,0); //non-local access
...
void PrintTrace(ofstream &RT){
	char St;
	RT<<"Rid arr_time chunk_id  deadline  fintime AP profit\n";
	for(int i=0;i<NUM_Req;i++){
	     if(R[i]->deadline>=R[i]->finish_time) St='H'; else St='M';
	     RT<<setw(4)<<i<<" "<<setw(6)<<R[i]->arrival_time<<" "<< setw(6)<<R[i]->cid 
	     	<<" "<<setw(8) <<R[i]->deadline <<" "<<setw(8)<<R[i]->finish_time<<setw(3)<<St
		<<setw(4) <<R[i]->AP<<" "<<setw(6)<<R[i]->profit<<endl;	
	}
...
void Generate_Request(){
	...
	for(int i=0;i<NUM_Req;i++){
		...
		if((random()%100)<20) { 
			...
			History.push_back(newcid);
		      } else newcid=History[random()%5];
		}

		R.push_back(new Req(ar, newcid, i, MIN_DEADLINE+random()%20, -1,random()%NUM_AP, random()%100));
		ar=ar+random()%10;
		rid++;
	}
	PrintTrace(ReqTrace);
	*/
	...
	int ar, newcid, deadline, AP, profit;
	int t;
	char tt;
	...
	for (int i=0;i<NUM_Req;i++)
	{
		...
		R.push_back(new Req(ar, newcid, i, deadline, -1,AP, profit));
...
void PrintStat(){
	...
	cout << "Hit Ratio = " << TGR*100/R.size() <<"\n";
	cout << "Total Profit = " << PGR <<"\n";
