...
void Generate_Request(){
	...
	for(int i=0;i<NUM_Req;i++){
		...
		if( randint >= 0 && randint <= HIGH_P)
			R.push_back(new Req(ar, newcid, i, HIGH_Deadline, -1,random()%NUM_AP, HIGH_Profit));
		else if (randint > HIGH_P && randint <= MID_P+HIGH_P)
			R.push_back(new Req(ar, newcid, i, MID_Deadline, -1,random()%NUM_AP, LOW_Profit));

		else
			R.push_back(new Req(ar, newcid, i, LOW_Deadline, -1,random()%NUM_AP, MID_Profit));
