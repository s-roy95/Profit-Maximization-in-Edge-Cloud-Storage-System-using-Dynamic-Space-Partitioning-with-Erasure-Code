...
#define HIGH_Deadline 40
#define MID_Deadline 50
#define LOW_Deadline 60
...
{    
    ...
    {
        int connect_limit=(rand()%(aP_eS_ub_dis-aP_eS_lb_dis+1))+aP_eS_lb_dis;
        set<int> eS_indexes;
...
{
    //Initialize pri_pub_ratio based on distribution:
    // 20% of ES: ratio 0.8 or 0.9
    // 50% of ES: ratio 0.7
    // 30% of ES: ratio 0.5 or 0.6
    
    int high_ratio_count = (no_eS * 20) / 100;  // 10% of ES
    int mid_ratio_count = (no_eS * 50) / 100;   // 50% of ES
    int low_ratio_count = no_eS - high_ratio_count - mid_ratio_count;  // remaining 30%
    
    int index = 0;
    
    // Assign high ratio (0.8 or 0.9) to first 20% ES
    for(int i=0; i<high_ratio_count; i++)
    {
        int choice = rand() % 2;  // randomly choose between 0.8 and 0.9
        pri_pub_ratio[index++] = (choice == 0) ? 0.8 : 0.9;
    }
    
    // Assign mid ratio (0.7) to next 50% ES
    for(int i=0; i<mid_ratio_count; i++)
    {
        pri_pub_ratio[index++] = 0.7;
    }
    
    // Assign low ratio (0.5 or 0.6) to remaining 30% ES
    for(int i=0; i<low_ratio_count; i++)
    {
        int choice = rand() % 2;  // randomly choose between 0.5 and 0.6
        pri_pub_ratio[index++] = (choice == 0) ? 0.5 : 0.6;
    }
	...
	FILE* fptr=fopen("eS_initialization.txt","w");
	if(fptr==NULL)
	    {
		cout<<"File Creation failed\n";
		exit(0);
    ...
    //store pri_pub_ratio values in file for reference
    FILE* fptr2=fopen("pri_pub_ratio.txt","w");
    if(fptr2==NULL)
    {
        cout<<"pri_pub_ratio File Creation failed\n";
        exit(0);
    }
    fprintf(fptr2,"ES_ID\tPrivate_Ratio\tPublic_Ratio\n");
    fprintf(fptr2,"-----\t-------------\t------------\n");
    for(int i=0;i<no_eS;i++)
    {
        fprintf(fptr2,"ES_%d\t%.2f (%.0f%%)\t%.2f (%.0f%%)\n",i,pri_pub_ratio[i],pri_pub_ratio[i]*100,(1-pri_pub_ratio[i]),(1-pri_pub_ratio[i])*100);
    }
    fclose(fptr2);
    
    //Print pri_pub_ratio values to console
    cout<<"\n=== Private:Public Ratio for Each Edge Server ===\n";
    cout<<"ES_ID\tPrivate_Ratio\tPublic_Ratio\n";
    cout<<"-----\t-------------\t------------\n";
    for(int i=0;i<no_eS;i++)
    {
        cout<<"ES_"<<i<<"\t"<<fixed<<setprecision(2)<<pri_pub_ratio[i]<<" ("<<(int)(pri_pub_ratio[i]*100)<<"%)";
        cout<<"\t"<<(1-pri_pub_ratio[i])<<" ("<<(int)((1-pri_pub_ratio[i])*100)<<"%)\n";
    }
    cout<<"=================================================\n\n";
...
{
	FILE* fptr=fopen("eS_initialization.txt","r");
	if(fptr==NULL)
	    {
		cout<<"File Opening failed\n";
		exit(0);
...
void Generate_Request()
{
    ...
    for (int i = 0; i < uniqueCount; i++)
     {
        // Generate unique filem values for each field
        int aP = rand() % no_aP; 
        int cid = rand() % no_movies;  
	...
	if(i%5==0 and i!=0) arrival_time+=rand()%10;
	id++;
    }
	...
	if(i%5==0) arrival_time=arrival_time+rand()%10;
...
void Load_Requests()
{
 ...
 unsigned int number_of_lines = 0;
	...
	//   printf("%u\n", number_of_lines);
	    ...
	    fclose(fptr);
	    FILE* fptr1=fopen("input_1.txt","r");
	if(fptr1==NULL)
	    {
		cout<<"File Opening failed\n";
		exit(0);
    ...
    {
        // Read in correct order matching file format: arr_time c_id deadline AP profit
        fscanf(fptr, "%d", &arrival_time);
        fscanf(fptr, "%d", &cid);
        fscanf(fptr, "%d", &deadline);  // deadline comes before AP in file
        fscanf(fptr, "%d", &aP);        // AP comes after deadline in file
        fscanf(fptr, "%d", &profit);
        
        // Auto-generate request_id as sequential number
        int id = i;
        
        R.push_back(new Req(id, arrival_time, cid, aP, deadline, profit));
        total_profit += profit;
    }
    fclose(fptr1);
}
...
void PutRequest2aP_nosplit_erasure(Req* curr)
{
    ...
    //temp_val is to decide whether its public or private replacement
    	...
    	//private replacement
    	if(temp_val<=pri_pub_ratio[selected_eS]) LRU(selected_eS,cid,data_chunks,0,pub_block_count_sno-1);
    	//public replacement
