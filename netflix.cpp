...
#define max_datachunks 4
#define req_datachunks 4
#define eS_blockcount 200
...
{
	int size=R.size();
	FILE* fptr=fopen("RequestTrace_0R.txt","w");
	if(fptr==NULL)
	{
...
void Load_Requests()
{
    ...
    FILE* fptr = fopen("RequestTrace_0R.txt", "r");
    if(fptr==NULL)
    {
        cout<<"File Opening failed\n";
        exit(0);
	...
	//FILE* fptr=fopen("netflix_data\\final_1.txt","r");
	
    FILE* fptr1 = fopen("RequestTrace_0R.txt", "r");
    if(fptr1==NULL)
    {
        cout<<"File Opening failed\n";
        exit(0);
    ...
    // Skip the first line
    char buffer[1024];
    if (fgets(buffer, sizeof(buffer), fptr1) == NULL) {
...
void PutRequest2aP_split_erasure(Req* curr)
{
 ...
 //select the edge server connected to aP based upon probability and random data generation
    ...
    temp_val=rand()%11;
    temp_val/=10;
    if(temp_val<=pri_pub_ratio) private_rep=true;
...
void Simulate_split_erasure()
{
    ...
    for(int i=0;i<NUM_req;i++)
    {
     if(i%50000==0) update_eS_aP_blockSpace();
        PutRequest2aP_split_erasure(R[i]);
        //on each 100th request
...
int main()
{
  ...
  Simulate_nosplit_erasure(); //this is actually our method SE
      //load_Infrastructure_Details();
       // load_eS_initialization();
